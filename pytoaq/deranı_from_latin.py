...
def deranı_from_latin(lt, opts = {}):
  ...
  MS = normalized_re_from_wordset(latin.root_subordinators)
  ...
  RRL = (  # Rewrite Rule List
    ...
    (f" (da)(?![{L}{T}])", r" \1 󱛕"),
    (f"(?<=[{L}{T}])(?<!mo)(?<!m[{T}]o)(?<!shu)(?<!sh[{T}]u)([󱛓󱛙]*)([^{L}{T}󱛒󱛙]*)([^{L}{T}󱛒󱛙]({MS})(?![{L}{T}])|$)",
     r"\1 󱛕\2\3"),
    ("󱛕 󱛕", "󱛕"),
